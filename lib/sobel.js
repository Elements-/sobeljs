...
module.exports = function(image, kernels) {
  var buf = greyscale.fromRGBA(image.data);
  var edges_buf = Buffer.alloc(buf.length);
  ...
  for(var y = 0; y < image.height; y++) {
    for(var x = 0; x < image.width; x++) {
      ...
      for(var kernel of kernels) {
        ...
        for(var kY = 0; kY < kernel.length; kY++) {
          for(var kX = 0; kX < kernel[kY].length; kX++) {
            ...
            intensity += buf[idx] * kernel[kY][kX];
          }
        }
      ...
      if(normalized_intensity < 32) normalized_intensity = 0;
  ...
  image.data = greyscale.toRGBA(edges_buf);
  return image;
}
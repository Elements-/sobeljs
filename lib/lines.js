module.exports = function(image) {
  ...
  for(var y = 0; y < image.height; y++) {
    for(var x = 0; x < image.width; x++) {
      ...
      if(image.data[idx] >= image.threshold) {
        ...
        for(var radians = 0; radians <= Math.PI; radians += 0.01) {
          ...
          for(var evalX = 0; evalX < image.width; evalX++) {
            ...
            if(image.data[idx] >= image.threshold) {
              ...
              else if(ranges[ranges.length - 1].end == evalX - 1) {
  ...
  equations = equations.filter(function(equation) {
    return equation.largest_range.size > 20;
  });
  ...
  for(var equation of equations) {
    ...
    for(var x = 0; x < image.width; x++) {
      if(x < equation.largest_range.start || x >= equation.largest_range.end) continue;
